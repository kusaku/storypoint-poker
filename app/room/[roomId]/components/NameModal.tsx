  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center">
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl p-6 max-w-md w-full m-4">
        <h2 className="text-2xl font-bold text-indigo-600 dark:text-indigo-400 mb-4">{t('nameModal.enterYourName')}</h2>
        <p className="text-gray-600 dark:text-gray-300 mb-4">{t('nameModal.pleaseEnterNameToJoin')}</p>
        <div className="space-y-4">
          <div>
            <label htmlFor="nameInput" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
              {t('nameModal.yourNameLabel')}
            </label>
            <input
              id="nameInput"
              type="text"
              ...
              placeholder={t('nameModal.enterYourNamePlaceholder')}
            ...
            {nameInput.trim().length === 0 && (
              <p className="mt-1 text-xs text-red-500 dark:text-red-400">{t('nameModal.nameRequired')}</p>
            )}
          </div>
          <button
            onClick={onEnterName}
